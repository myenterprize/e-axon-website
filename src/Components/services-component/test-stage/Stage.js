...
const Stage = ({stageData, elementColor}) => {
  ...
  const stage = stageData.map((item, i) => {
    return (
      <div key={i} className='w-full border-b-[10px] border-[#ffffff] last:border-none md:border-none md:max-w-[550px] px-[40px] py-[16px] lg:px-[58px] mb-5'>
  ...
  return (
    <div className='scroll-section w-full h-auto overflow-y-visible md:w-[50%] md:max-h-[118vh] md:overflow-y-auto mb-[24px]'>
      {stage}
    </div>
  )
}
