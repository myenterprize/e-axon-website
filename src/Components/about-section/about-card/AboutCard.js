...
import CardVision from '@/assets/images/Animation images/card-1.gif'
import CardMission from '@/assets/images/Animation images/card-2.gif'
...
const AboutCard = ({title, desc, bgColor, padding, userBg, gifShow}) => {
  ...
  return (
    <div className='mb-[16px] h-[396px] w-full md:max-w-[440px] md:h-[440px] sm:rounded-[8px] bg-S1-500 flex flex-col justify-between items-center relative py-[48px] px-[33px] text-center z-20' style={{backgroundColor: bgColor}}>
      <div className={`user-top w-[150px] h-[150px] absolute bottom-[150px] bg-P2-900 rounded-[50%] ${gifShow ? "visible" : "hidden"}`} style={{ backgroundImage: `url(${CardVision.src})`, backgroundPosition: 'center', backgroundSize: 'cover'}}></div>
      <div className={`user-top w-[150px] h-[150px] absolute bottom-[150px] ${gifShow ? "hidden" : "visible"}`} style={{ backgroundImage: `url(${CardMission.src})`, backgroundPosition: 'center', backgroundSize: 'cover'}}></div>
